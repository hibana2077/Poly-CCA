...
def experiment_failure_map(out_dir: Path, motif_probs=None, sample_sizes=None):
    ...
    df = pd.DataFrame(results)
    # Use keyword arguments for pivot to avoid TypeError (pandas requires explicit naming)
    pivot_rho = df.pivot(index='motif_prob', columns='N', values='rho1')
    pivot_f1 = df.pivot(index='motif_prob', columns='N', values='F1')
    plt.figure(figsize=(6,4))
